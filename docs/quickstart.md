...
4. **Inspect.** `model.subject_params()` returns MAP estimates in natural space, one row per subject. `result.posterior_mu` is the fitted group-level mean (in Gaussian space), and `result.convergence` reports whether the EM loop converged. Also `model.outfit` contains the per-subject diagnostics dictionary returned by `rw1a1b_fit(..., output="all")`, which includes the negative log-likelihood and any other trialwise quantities you choose to return.

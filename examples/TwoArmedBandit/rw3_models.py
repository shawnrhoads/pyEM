...
def simulate(params, nblocks=3, ntrials=24, outcomes=None):
    ...
    for subj_idx in range(nsubjects):
        beta, lr, rew_sensitivity = params[subj_idx,:]
        ...
        for b in range(nblocks): # if nblocks == 1, then use reversals
            for t in range(ntrials):
                ...
                # calculate PE
                PE[subj_idx, b, t] = (rew_sensitivity * rewards[subj_idx, b, t]) - EV[subj_idx, b, t, c]
...
def fit(params, choices, rewards, prior=None, output='npl'):
    ...
    rew_sensitivity = norm2alpha(params[2])
    ...
    if lr < min(this_alpha_bounds) or lr > max(this_alpha_bounds):
        return 10000000
    this_rew_bounds = [0, 1]
    if rew_sensitivity < min(this_rew_bounds) or rew_sensitivity > max(this_rew_bounds):
        return 10000000
    ...
    for b in range(nblocks):
        for t in range(ntrials):
            ...
            # calculate PE
            PE[b, t] = (rew_sensitivity * rewards[b, t]) - EV[b, t, c]
    ...
    if (output == 'npl') or (output == 'nll'):
        ...
        subj_dict = {'params'     : [beta, lr, rew_sensitivity],
                     'choices'    : choices, 
                     'rewards'    : rewards, 
                     'EV'         : EV, 
                     'CH_PROB'    : CH_PROB, 
